...
public class CurpManagerPool {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("\n===== CURPs ORDENADAS (usando ThreadPool) =====");

        // Enviar cada tarea de ordenamiento al pool
        for (int i = 0; i < listas.size(); i++) {
            int index = i; // necesario porque se usa dentro de lambda
            pool.execute(() -> {
                ArrayList<String> ordenadas = ordenarCurps(listas.get(index));
                System.out.println("Lista " + (index + 1) + ": " + ordenadas +
                        " (procesada por " + Thread.currentThread().getName() + ")");
            });
        }
